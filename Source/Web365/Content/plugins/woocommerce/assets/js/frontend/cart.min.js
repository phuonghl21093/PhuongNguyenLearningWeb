jQuery(function (a) {
    ...
    var b = function (a) {
        ...
        c(a) || a.addClass("processing").block({message: null, overlayCSS: {background: "#fff", opacity: .6}})
    }, e = function (a) {
        a.removeClass("processing").unblock()
    }, f = function (b) {
        var c = a.parseHTML(b), d = a(".shop_table.cart", c).closest("form"), e = a(".cart_totals", c), f = a(".woocommerce-error", c), i = a(".woocommerce-message", c);
        if (a(".woocommerce-error, .woocommerce-message").remove(), 0 === d.length) {
            ...
            a(".shop_table.cart").closest(".woocommerce").replaceWith(j), f.length > 0 ? h(f, a(".cart-empty").closest(".woocommerce")) : i.length > 0 && h(i, a(".cart-empty").closest(".woocommerce"))
        } else a(".woocommerce-checkout").length && a(document.body).trigger("update_checkout"), a(".shop_table.cart").closest("form").replaceWith(d), a(".shop_table.cart").closest("form").find('input[name="update_cart"]').prop("disabled", !0), f.length > 0 ? h(f) : i.length > 0 && h(i), g(e);
        a(document.body).trigger("updated_wc_div")
    }, g = function (b) {
        a(".cart_totals").replaceWith(b), a(document.body).trigger("updated_cart_totals")
    }, h = function (b, c) {
        c || (c = a(".shop_table.cart").closest("form")), a(".woocommerce-error, .woocommerce-message").remove(), c.before(b)
    }, i = {
        init: function (b) {
            ...
            var i = {security: wc_cart_params.update_shipping_method_nonce, shipping_method: h};
            a.ajax({
                type: "post", url: b("update_shipping_method"), data: i, dataType: "html", success: function (a) {
                    g(a)
                }, complete: function () {
            ...
            d(a("div.cart_totals")), d(c), a("<input />").attr("type", "hidden").attr("name", "calc_shipping").attr("value", "x").appendTo(c), a.ajax({
                type: c.attr("method"),
                url: c.attr("action"),
                data: c.serialize(),
                dataType: "html",
        ...
        init: function () {
            ...
            d(a("div.cart_totals")), a.ajax({
                url: b("get_cart_totals"), dataType: "html", success: function (a) {
                    g(a)
                }, complete: function () {
                    e(a("div.cart_totals"))
                }
            ...
            var d = a(b.currentTarget), e = a(document.activeElement), f = a("input[type=submit][clicked=true]");
            return 0 !== d.find(".shop_table.cart").length && (!c(d) && void(f.is('[name="update_cart"]') || e.is("input.qty") ? (b.preventDefault(), this.quantity_update(d)) : (f.is('[name="apply_coupon"]') || e.is("#coupon_code")) && (b.preventDefault(), this.apply_coupon(d))))
            ...
            d(c);
            var f = this, g = a("#coupon_code"), i = g.val(), j = {
                security: wc_cart_params.apply_coupon_nonce,
                coupon_code: i
            };
            a.ajax({
                type: "POST", url: b("apply_coupon"), data: j, dataType: "html", success: function (b) {
            ...
            c.preventDefault();
            var f = this, g = a(c.currentTarget).parents("tr"), i = a(c.currentTarget).attr("data-coupon");
            d(g.parents("table"));
            var j = {security: wc_cart_params.remove_coupon_nonce, coupon: i};
            a.ajax({
                type: "POST", url: b("remove_coupon"), data: j, dataType: "html", success: function (b) {
            ...
            b.preventDefault();
            var c = a(b.currentTarget), g = c.parents("form");
