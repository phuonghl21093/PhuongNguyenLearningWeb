    ...
    return {
        ...
        link: function (scope, element, attrs) {
            ...
            scope.sortData = function () {
                ...
                $(element).closest('.controls').find('input:hidden').val(scope.arrId);
...
'use strict';
web365app.directive('uploadFiles', function (utilityServices, fileService, fileTypeService) {
    return {
        restrict: 'A',
        link: function (scope, element, attrs) {

            scope.listTreeTypeFile = [];

            fileTypeService.getListForTree().then(function (response) {

                scope.listTreeTypeFile = response.list;

            }, function (err) {
                console.log(err.error_description);
            });

            utilityServices.loadTemplate('/Angular/Views/Admin/File/preview-upload-file.html').then(function (res) {

                $(element).find('.dropzone').dropzone({
                    url: "/Upload/UploadFile.ashx",
                    addRemoveLinks: true,
                    previewTemplate: res,
                    success: function (file, r, e) {

                        if (file.previewElement) {

                            file.previewElement.classList.add("dz-success");

                            $(file.previewElement).find('input:hidden[name="FileName"]').val(r);

                            $(file.previewElement).find('input:text[name="Name"]').val(r);
            ...
            scope.saveFiles = function () {
                ...
                if (typeId == '' || typeId == null || typeof typeId === 'undefined ') {
                    ...
                    utilityServices.notificationWarning('Thông báo', 'Bạn cần chọn nhóm file');

                } else {

                    $(element).find('.upload-file').each(function (i) {

                        $(this).prepend('<input type="hidden" name="TypeID" value="' + typeId + '" />');

                        fileService.modified($(this).serialize()).then(function (res) {

                            total += 1;

                            if (total == $(element).find('.upload-file').size()) {
            ...
            scope.closeUploadFiles = function () {
                ...
                scope.modalUploadFile.dismiss('cancel');

                if (typeof scope.selectFile !== 'undefined') {
                    scope.selectFile();
                }

                if (typeof scope.resetListFile !== 'undefined') {
                    scope.resetListFile();
                }
    ...
    return {
        ...
        link: function (scope, element, attrs) {
            ...
            scope.choiceFile = function ($event, item) {
                ...
                var newItem = $compile('<li class="item-sort-able" data-id="' + item.ID + '">' +
                                '<a href="/UploadFile/ImageThumbs/' + item.FileName + '" data-rel="colorbox">' +
                                    '<img alt="' + item.Alt + '" src="/Content/Admin/css/images/icon-file-zip.png" />' +
            ...
            scope.addNewFile = function () {
                ...
                scope.modalUploadFile = $modal.open({
                    templateUrl: 'Angular/Views/Admin/File/upload-files.html',
                    scope: scope
                });
