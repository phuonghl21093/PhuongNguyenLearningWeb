...
web365app.controller('baseController', ['$rootScope', '$scope', '$http', '$sce', '$timeout', '$modal', 'utilityServices',
  function ($rootScope, $scope, $http, $sce, $timeout, $modal, utilityServices) {
      ...
      $scope.selectLanguage = function (data) {
          utilityServices.getListLanguage().then(function (res) {
              if (res.data && res.data.length > 0) {
                  if (res.data.length === 1) {
                      $scope.translate(data, res.data[0].ID);
                  } else {
                      $scope.modalSelectLanguage = $modal.open({
                          templateUrl: '/Angular/Views/Admin/select-language.html',
                          size: 'sm',
                          scope: $scope,
                          controller: function () {
                              ...
                              $scope.select = function () {
                                  $scope.translate(data, $scope.languageSelected.id);
                                  $scope.dismiss();
                              }

                              $scope.dismiss = function () {
                                  $scope.modalSelectLanguage.dismiss('cancel');
                              }
                          }
                      });
                  }
