...
web365app.factory('articleService', ['$q', '$http', function ($q, $http) {
    ...
    var _getMultiLanguageList = function (id) {

        var deferred = $q.defer();

        //get data from services
        $http.get('/Article/MultiLanguageList?id=' + id).success(function (data) {
            deferred.resolve(data);
        }).error(function (err, status) {
            deferred.reject(err);
        });
    ...
    var _getEditForm = function (id, language) {
    ...
    articleService.getMultiLanguageList = _getMultiLanguageList;
