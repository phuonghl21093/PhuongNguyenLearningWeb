    ...
    public static class Web365Utility
    {
        ...
        public static string GetPathPicture(string fileName)
        {
            try
            {
                ...
                //    return ConfigWeb.ImagePath + "no_image.png";
        ...
        public static string GetTypeOfProject(int TypeId)
        {
            var returnString = string.Empty;
            try
            {
                var ProjectTMDV = ConfigurationManager.AppSettings["ProjectTMDV"];
                if (ProjectTMDV.IndexOf(TypeId.ToString()) >= 0)
                {
                    returnString = "management";
                }
                var ProjectBDS = ConfigurationManager.AppSettings["ProjectBDS"];
                if (ProjectBDS.IndexOf(TypeId.ToString()) >= 0)
                {
                    returnString = "renovation";
                }
                var ProjectKCN = ConfigurationManager.AppSettings["ProjectKCN"];
                if (ProjectKCN.IndexOf(TypeId.ToString()) >= 0)
                {
                    returnString = "construction";
                }
                var ProjectGT = ConfigurationManager.AppSettings["ProjectGT"];
                if (ProjectGT.IndexOf(TypeId.ToString()) >= 0)
                {
                    returnString = "interior";
                }
                var ProjectCTDD = ConfigurationManager.AppSettings["ProjectCTDD"];
                if (ProjectCTDD.IndexOf(TypeId.ToString()) >= 0)
                {
                    returnString = "dandung";
                }
                var ProjectKHAC = ConfigurationManager.AppSettings["ProjectKHAC"];
                if (ProjectKHAC.IndexOf(TypeId.ToString()) >= 0)
                {
                    returnString = "another";
                }
                return returnString;
            }
            catch (Exception)
