...
namespace Web365DA.RDBMS.Back_End.Repository
{
    ...
    public class OrderDABERepository : BaseBE<tblOrder>, IOrderDABERepository
    {
            ...
            bool isDelete = false)
        {
            ...
            {
                query = query.Where(p => p.CustomerName.ToLower().Contains(customerName));
            }

            if (!string.IsNullOrEmpty(email))
            {
                query = query.Where(p => p.Email.ToLower().Contains(email));
            }

            if (!string.IsNullOrEmpty(phone))
            {
                query = query.Where(p => p.Phone.ToLower().Contains(phone));
            }
